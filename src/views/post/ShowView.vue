...
onMounted(async () => {
  try {
    ...
    const result: AxiosResponse<Fetch> = await api.get<Fetch>(`post/${route.params.postId}`)
    const response: PostWithPostTypesAndPostFile = result.data.data as PostWithPostTypesAndPostFile
    form.name = response.name
    form.content = response.content
    form.meme_types = response.post_types.map((postType) => postType.meme_type)
    console.log(result)
  } catch (error) {
  ...
  try {
    ...
    memeTypes.value = result.data.data as MemeType[]
  } catch (error) {
    const err = error as AxiosError
    console.log(err)
  } finally {
...
const send = async () => {
  try {
    const result: AxiosResponse<Fetch> = await api.put<Fetch>(`post/${route.params.postId}`, {
      name: form.name,
    ...
    if (form.file) {
      const responsePost: Post = result.data.data as Post
      const formData = new FormData()
      formData.append('file', form.file)

      await apiStorage.patch(`storage/post/${responsePost.id}/post_file`, formData)
    }
  ...
  <DashboardLayout>
    <template #header>
      <div class="flex justify-between">
        <div>
          <h2 class="font-semibold text-xl text-gray-800 leading-tight">Show Post</h2>
        </div>
      </div>
    </template>
